...
Aplicación CRUD de tareas desarrollada con **HTML, CSS y JavaScript Vanilla**, utilizando `fetch()` y `JSON Server` para simular una API REST.
...
const API_BASE_URL = 'http://localhost:3007/todos';
...
const API_BASE_URL = 'http://10.5.225.213:3005/todos'; // Cambia esta URL por la de tu JSON Server
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // ===========================================
  // VALIDACIÓN DE USUARIO — NUEVO (agregado)
  // Inicia la pantalla de login antes de mostrar
  // la app. No modifica nada de lo anterior.
  // ===========================================
  initLoginScreen();
});
...
// =============================================
// 9) VALIDACIÓN DE USUARIO — NUEVO (agregado)
// Controla la pantalla de login y el acceso
// a la app. No modifica ninguna función anterior.
// =============================================

/*
Flujo:
1. Al cargar, se muestra #loginScreen y se oculta #appContainer.
2. El usuario escribe su nombre y hace clic en "Ingresar" (o presiona Enter).
3. Se valida que el campo no esté vacío.
4. Se precarga el campo userName del formulario con el nombre ingresado
   y se deshabilita para que no se pueda cambiar dentro de la app.
5. Se muestra #appContainer, se oculta #loginScreen y se llama loadTodos().
6. El botón "Cerrar sesión" revierte todo al estado inicial.
*/
...
function initLoginScreen() {
  ...
  const loginError     = document.getElementById('loginUserNameError');
  ...
  function handleLoginSubmit() {
    ...
    if (!nombre) {
      loginError.textContent = 'El nombre de usuario es obligatorio.';
    ...
    // Precargar y bloquear el campo userName del formulario principal
    // para que siempre coincida con quien inició sesión
    userNameInput.value    = nombre;
    userNameInput.readOnly = true;

    // Mostrar badge con nombre en el header
    headerUserBadge.textContent = `👤 ${nombre}`;
  ...
  function handleLogout() {
    ...
    // Limpiar campo userName del formulario
    userNameInput.value    = '';
    userNameInput.readOnly = false;

    // Limpiar campo del login para la próxima vez
    loginInput.value = '';
    loginError.textContent = '';
    loginInput.classList.remove('error');

    // Limpiar badge del header
    headerUserBadge.textContent = '';
